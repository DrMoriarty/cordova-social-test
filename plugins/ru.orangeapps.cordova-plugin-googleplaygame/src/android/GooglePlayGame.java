...
public class GooglePlayGame extends CordovaPlugin implements ConnectionCallbacks, OnConnectionFailedListener {
    ...
    public static final String ARGUMENT_DEEPLINK = "deeplink";
    public static final String ARGUMENT_IMAGE = "image";
    public static final String ARGUMENT_ACTIONTEXT = "actiontext";
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        ...
        if (ACTION_LOGIN.equals(action) || ACTION_TRY_SILENT_LOGIN.equals(action)) {
        ...
        if (ACTION_IS_AVAILABLE.equals(action)) {
            ...
            // invite
            JSONObject obj = args.getJSONObject(0);
            String title  = obj.optString(ARGUMENT_TITLE, "");
            String message = obj.optString(ARGUMENT_MESSAGE, null);
            String deeplink = obj.optString(ARGUMENT_DEEPLINK, null);
            String image = obj.optString(ARGUMENT_IMAGE, null);
            String actiontext = obj.optString(ARGUMENT_ACTIONTEXT, null);

            Intent intent;
            AppInviteInvitation.IntentBuilder builder = new AppInviteInvitation.IntentBuilder(title);
            if(message != null) builder.setMessage(message);
            if(deeplink != null) builder.setDeepLink(Uri.parse(deeplink));
            if(image != null) builder.setCustomImage(Uri.parse(image));
            if(actiontext != null) builder.setCallToActionText(actiontext);
            this.cordova.getActivity().startActivityForResult(builder.build(), REQUEST_INVITE);
        } else {
            return false;
        }
        return true;
    ...
    public void onActivityResult(int requestCode, final int resultCode, final Intent intent) {
        ...
        if(requestCode == REQUEST_INVITE) {
            if(resultCode == Activity.RESULT_OK) {
                ...
                Log.d(TAG, "User sended invites:" + ids);
                savedCallbackContext.success(ids.toString());
            } else {
                Log.d(TAG, "Invite error code:"+resultCode);
                savedCallbackContext.error("GP invite error:"+resultCode);
            }
        } else {
